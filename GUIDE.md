...
| `carousel.html` | Carrousel générique des 6 galeries (paramètre `?c=`) | Rarement |
...
images/
  ...
  galerie/              photo-1..6.jpg = les 6 pavés (logo « appareil photo »)
                        sv/  = carrousel « Souvenirs »   (sv-1, sv-2, …)
                        sr/  = carrousel « Sorties »     (sr-1, sr-2, …)
                        tt/  = carrousel « Tennis de table » (tt-1, tt-2, …)
                        js/  = carrousel « Jumelage »    (js-1, js-2, …)
                        du/  = carrousel « Deutsch Unterricht » (du-1, …)
                        db/  = carrousel « Dîner en blanc »     (db-1, …)
```
...
### Galerie : 6 pavés, 6 carrousels

La galerie présente **6 pavés** ; chacun ouvre un **carrousel** (la page
`carousel.html`) qui affiche automatiquement les photos de son dossier :

| Pavé | Dossier des photos | Préfixe des noms |
|------|--------------------|------------------|
| Souvenirs | `images/galerie/sv/` | `sv-` |
| Sorties | `images/galerie/sr/` | `sr-` |
| Match Tennis de Table | `images/galerie/tt/` | `tt-` |
| Jumelage | `images/galerie/js/` | `js-` |
| Deutsch Unterricht | `images/galerie/du/` | `du-` |
| Dîner en blanc | `images/galerie/db/` | `db-` |

Tous les carrousels fonctionnent à l'identique (flèches, points, miniatures,
glissement, clavier, « Retour ») et sont **dynamiques** : pour ajouter,
retirer ou remplacer des photos, il suffit de déposer les fichiers dans le
dossier correspondant — sur GitHub Pages, n'importe quel nom convient (tri par
nom) ; ailleurs, utiliser des noms numérotés qui se suivent (`sv-1.jpg`,
`sv-2.jpg`, …). Un dossier vide affiche « Aucune photo pour le moment. ».

Les catégories (titre, dossier, préfixe) sont définies en tête du script de
`carousel.html`, sous `CATEGORIES` ; pour créer une nouvelle catégorie :
créer le dossier, y déposer les photos, ajouter une entrée dans `CATEGORIES`
et un pavé dans `galerie.liste` (config.js).
